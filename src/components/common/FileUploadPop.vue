            ...
            <div class="upload-file-body" v-show="!listState">
                ...
                >
                    <div class="upload-file-con">
                        ...
                        <div class="text3">{{ hintText }}</div>
                    </div>
                </el-upload>
                <div class="upload-func">
                    <div>Selected Files: {{ imageList.length + fileList.length }}</div>
                    <div class="func-end">
                        <el-button
                            class="func-button1 hover"
                            @click="listState = true"
                        >
                            View/Edit Selected
                        </el-button>
                    </div>
                ...
                <div class="list-func">
                    ...
                    <div class="func-end">
                        ...
                        <el-button
                            class="func-button1 hover"
                            @click="uploadFile"
                            :disabled="dialogData.loading || allLenght == 0"
                        >
                            Upload
                            <i
...
<script>
export default {
    props: ["uploadAllCallback", "filePopConfig", "localUploadLimit",'uploadPress','hintText'],
    data() {
        return {
            listState: false,
            imageList: [],
    ...
    computed: {
        allLenght(){
            return this.imageList.length + this.fileList.length
        },
        dialogData() {
    ...
    methods: {
        ...
        async uploadFile() {
            ...
            // 文件上传前校验
            for (let i of files) {
                let state = await this.localUploadBefore(i);
                if (state) {
                    return true;
                }
            }
        ...
        async uploadFileToServer(file, func) {
            // let src = "";
            // 上传
            ...
            let urlObj = {}
            if(this.uploadPress){
              urlObj = await this.$uploadFile().comPress({file,dirKey:file.dirKey})
            }else{
              urlObj = await this.$uploadFile().fireBase({file,dirKey:file.dirKey})
            }
            if(urlObj.error){
              func("error");
            }else{
              func(this.uploadPress?urlObj.fileUrl:urlObj.fileRelativeUrl)
            }
        },
    },
    watch: {
...
.file-prop >>> .el-upload-dragger:hover{
    border-color: #d9d9d9;
}
...
.file-prop >>> .upload-file-con .text3 {
    width: 400px;
    font-size: 13px;
    margin-top: 5px;
    word-break:keep-all;
